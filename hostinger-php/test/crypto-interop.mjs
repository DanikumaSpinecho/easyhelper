// Test d'interopérabilité du chiffrement côté navigateur (crypto.js).
//
...
//
// Usage : node hostinger-php/test/crypto-interop.mjs
...
async function main() {
  ...
  assert.equal(aRaw[0], 4, 'préfixe 0x04 attendu par le serveur');
  ...
  tampered[20] ^= 0x01;
  const bad = await bob.decryptFrame(new Uint8Array(tampered));
  assert.equal(bad, null, 'une altération du cadre est détectée et refusée');
  ...
  const nodeSide = createECDH('prime256v1');       // « l'autre poste »
  nodeSide.generateKeys();
  const nodePub = nodeSide.getPublicKey().toString('base64');

  const derivedInBrowser = await alice.derive(nodePub, salt);
  assert.ok(derivedInBrowser, 'clé dérivée face à une clé externe');

  const shared = nodeSide.computeSecret(aRaw);     // secret partagé côté node
  ...
  const otherKey = await bob.derive(aPub, randomBytes(16).toString('hex'));
  assert.ok(otherKey, 'dérivation avec un autre sel');
  const wrong = await bob.decryptFrame(await alice.encryptFrame(message));
  assert.equal(wrong, null, 'un sel différent ne permet pas de déchiffrer (clés distinctes)');
